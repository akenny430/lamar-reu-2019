---
title: "Code for Leuk Data Set Visuals"
...
# making paletts longer
cb_pal_full = c(cb_pal, cb_pal, cb_pal[1])
cb_pal_25_full = c(cb_pal_25, cb_pal_25, cb_pal_25[1])
cb_pal_50_full = c(cb_pal_50, cb_pal_50, cb_pal_50[1])
...
# Reading in the leuk data and results
...
enet_ROC_s = roc(response = act, predictor = as.vector(enet_prob), plot = FALSE, smooth = FALSE) # error
```
...
glask_ROC_s = roc(response = act, predictor = as.vector(glask_prob), plot = FALSE, smooth = FALSE) # error
```
...
sglk_ROC_s = roc(response = act, predictor = as.vector(sglk_prob), plot = FALSE, smooth = FALSE) # error
```
...
pclk_ROC_s = roc(response = act, predictor = as.vector(pclk_prob), plot = FALSE, smooth = FALSE) # error
```
...
glash_ROC_s = roc(response = act, predictor = as.vector(glash_prob), plot = FALSE, smooth = FALSE) # error
```
...
pclh_ROC_s = roc(response = act, predictor = as.vector(pclh_prob), plot = FALSE, smooth = FALSE) # false
```
...
par(font.main = 1)
title(main = "K = 19", cex.main = 1.25, line = 1)
...
par(font.main = 1)
title(main = "K = 5", cex.main = 1.25, line = 1)
...
pch_seq = c(0:(opt - 1))
for(i in 1:opt){
  points(seq(1, length(gsvdk[[i]]), 1), gsvdk[[i]], pch = pch_seq[i], col = cb_pal_full[i], type = "b")
}

# legend
legend("topright", paste("Group ", c(1:opt), sep = ""), pch = pch_seq, col = cb_pal_full, 
       inset = 0.025, bg = "white", cex = 1.25, ncol = 2)

dev.off()
```

```{r plot eigenvalues h}
pdf(file = "leuk_eigen_h.pdf")
...
#opt = maxSE(f = k_mean$Tab[, "gap"], SE.f = k_mean$Tab[, "SE.sim"]); opt
opt = 19

# gives k = 19 as optimal amount of clusters

pdf(file = "leuk_gap_stat.pdf")

par(family = "serif", mar = c(5.1, 5.1, 4.6, 2.1), font.main = 2)
plot(k_mean, las = 1, xlim = c(0, 20), ylim = c(1, 1.5),
     xlab = "m", main = "", ylab = "", cex.axis = 1.5, cex.lab = 1.75, cex = 1, pch = 1,
     do.arrows = TRUE, arrowArgs = list(col = "red3", length = 1/12, angle = 90, code = 3))
title(main = "Leukemia Data Set Gap Statistic", line = 2.75, cex.main = 2)
title(ylab = "Gap(m)", line = 3.75, cex.lab = 1.75)
par(font.main = 1)
title(main = "B = 500, nstart = 20, M = 20, K = 19", cex.main = 1.35, line = 1)

# legend
c("Gap(m)", expression(paste(delta, "(m)", sep = "")))
...
segments(x0 = opt, y0 = 1.5, x1 = opt, y1 = k_mean$Tab[opt, 3] + k_mean$Tab[opt, 4] + 0.0125,
         col = "black", lty = 3)
segments(x0 = opt, y0 = r$top + 0.0125, x1 = opt, y1 = k_mean$Tab[opt, 3] - k_mean$Tab[opt, 4] - 0.015,
         col = "black", lty = 3)
...
```{r cluster plot}
set.seed(1)
k_clust = kmeans(x, centers = opt, nstart = 20, iter.max = 30)
...
#         col.p = c(cb_pal, cb_pal, cb_pal[1])[k_clust$cluster], cex.axis = 1.5, cex.lab = 1.75)
# These two components explain 83.63% of the point variability
...
pdf(file = "leuk_clus_plot.pdf")

# making the plot
par(family = "serif", mar = c(5.1, 5.1, 4.6, 2.1), font.main = 2)
plot(PC[,1], PC[,2], las = 1, pch = c(0:18)[k_clust$cluster], col = cb_pal_full[k_clust$cluster],
     cex.axis = 1.5, cex.lab = 1.75, xlab = expression(bold(z)[1]), ylab = expression(bold(z)[2]))
title(main = "Leukemia Data Set Cluster Plot", line = 2.75, cex.main = 2)
par(font.main = 1)
title(main = "These two components explain 83.63% of the point variability", cex.main = 1.35, line = 1)
...
for(i in 1:opt){
  polygon(x = clus_bor[[i]][, 1], y = clus_bor[[i]][, 2], col = cb_pal_25_full[i], border = NA)
}

# labels on plot
ex = c(rep(0.7, 9), rep(1.2, 10)); ey = 0.35
for(i in 1:opt){
  rect(cent[i, 1] - ex[i], cent[i, 2] - ey, cent[i, 1] + ex[i], cent[i, 2] + ey,
...
pdf(file = "leuk_den.pdf", width = 8, height = 5)
...
A2Rplot(method, k = nch, boxes = FALSE, col.up = "black",
        ...
        main = "")
abline(h = 0.875, col = "black", lty = 2)
title(main = "Leukemia Data Set Dendrogram", line = 0.5)
par(font.main = 1)
title(main = "Using Complete Linkage with Correlation as Dissimilarity", line = -20.65, cex.main = 0.9)
title(main = "K = 5", line = -22, cex.main = 0.9)
